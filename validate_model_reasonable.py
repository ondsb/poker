#!/usr/bin/env python3
"""
Comprehensive validation script to test if the model gives reasonable predictions.
This script tests scenarios that should have clear winners and explains the normalization issue.
"""
...
def explain_normalization_issue():
    """Explain why normalization is problematic."""
    print("🔍 UNDERSTANDING THE NORMALIZATION ISSUE")
    print("=" * 60)
    print()
    print("❌ PROBLEM: The current app normalizes probabilities, which is WRONG!")
    print()
    print("📊 How the model was trained:")
    print("   - Target: 'is_winner' = 1 if player has highest finishing stack")
    print("   - Only ONE player wins each hand (gets entire pot)")
    print("   - Model predicts P(player wins entire pot)")
    print()
    print("🎯 What the model outputs:")
    print("   - Raw probabilities: P(Player 1 wins), P(Player 2 wins), etc.")
    print("   - These are independent probabilities for each player")
    print("   - They do NOT sum to 1 across players!")
    print()
    print("❌ Why normalization is wrong:")
    print("   - Normalizing makes it seem like probabilities sum to 1")
    print("   - But in reality, multiple players can have high win probabilities")
    print("   - Example: AA vs KK vs QQ - all have significant chances")
    print()
    print("✅ Correct interpretation:")
    print("   - Each probability = chance that player wins the entire pot")
    print("   - Higher probability = better chance of winning")
    print("   - Compare probabilities directly (don't normalize)")
    print()

def test_clear_winner_scenarios():
    """Test scenarios that should have very clear winners."""
    ...
    # Define test scenarios with clear winners
    scenarios = [
        {
            "name": "Pocket Aces vs 7-2 Offsuit",
            ...
            "expected_winner": 0,
            "expected_min_prob": 0.80  # AA should have at least 80% chance
        },
        {
            ...
            "board_cards": ["unknown", "unknown", "unknown", "unknown", "unknown"],
            "expected_winner": 0,
            "expected_min_prob": 0.75  # KK should have at least 75% chance
        },
        {
            ...
            "expected_min_prob": 0.85  # Set should have at least 85% chance
        },
        {
            "name": "Straight vs High Cards",
            "description": "Made straight should be heavily favored",
            "player_cards": [("8h", "9h"), ("As", "Kh")],
            "board_cards": ["7d", "6c", "5s", "unknown", "unknown"],
            "expected_winner": 0,  # 56789 straight vs AK
            "expected_min_prob": 0.80  # Straight should have at least 80% chance
        },
        {
            "name": "Flush vs High Cards",
            "description": "Made flush should be heavily favored",
            "player_cards": [("Ah", "Kh"), ("As", "Kd")],
            "board_cards": ["2h", "7h", "9h", "unknown", "unknown"],
            "expected_winner": 0,  # AhKh flush vs AsKd
            "expected_min_prob": 0.85  # Flush should have at least 85% chance
        },
        {
            "name": "Full House vs Two Pair",
            "description": "Full house should be heavily favored",
            "player_cards": [("Ah", "Ad"), ("Ks", "Kh")],
            "board_cards": ["Ac", "Kd", "2s", "unknown", "unknown"],
            "expected_winner": 0,  # AAAKK full house vs KKAA two pair
            "expected_min_prob": 0.90  # Full house should have at least 90% chance
        },
        {
            "name": "Three Players - AA vs KK vs 72o",
            "description": "AA should be heavily favored in 3-way pot",
            "player_cards": [("As", "Ah"), ("Ks", "Kh"), ("7c", "2d")],
            "board_cards": ["unknown", "unknown", "unknown", "unknown", "unknown"],
            "expected_winner": 0,
            "expected_min_prob": 0.65  # AA should have at least 65% chance in 3-way
        }
    ]
    ...
    for i, scenario in enumerate(scenarios):
        ...
        # Display results
        print(f"   Results (Raw Probabilities - NO NORMALIZATION):")
        for j, (cards, prob) in enumerate(zip(scenario['player_cards'], raw_probs)):
            print(f"     Player {j+1} ({cards[0]} {cards[1]}): {prob:.1%}")
        
        # Check if prediction matches expectation
        predicted_winner = np.argmax(raw_probs)
        expected_winner = scenario['expected_winner']
        predicted_prob = raw_probs[expected_winner]
        expected_min_prob = scenario['expected_min_prob']
        
        is_correct = predicted_winner == expected_winner
        is_reasonable = predicted_prob >= expected_min_prob
        
        status = "✅ PASS" if is_correct and is_reasonable else "❌ FAIL"
        print(f"   Expected: Player {expected_winner + 1} wins with ≥{expected_min_prob:.1%} probability")
        print(f"   Predicted: Player {predicted_winner + 1} wins with {predicted_prob:.1%} probability")
        print(f"   Status: {status}")
        
        if not is_reasonable:
            print(f"   ⚠️  Warning: Expected probability too low!")
        
        results.append({
            'scenario': scenario['name'],
            'expected_winner': expected_winner,
            'predicted_winner': predicted_winner,
            'expected_min_prob': expected_min_prob,
...
def test_probability_sum_analysis():
    """Test if probabilities sum to 1 (they shouldn't)."""
    ...
    test_cases = [
        {
            "name": "Heads-up: AA vs KK",
            "player_cards": [("As", "Ah"), ("Ks", "Kh")],
            "board_cards": ["unknown", "unknown", "unknown", "unknown", "unknown"]
        },
        {
            "name": "Three-way: AA vs KK vs QQ",
            "player_cards": [("As", "Ah"), ("Ks", "Kh"), ("Qs", "Qh")],
            "board_cards": ["unknown", "unknown", "unknown", "unknown", "unknown"]
        },
        {
            "name": "Six-way: AA vs KK vs QQ vs JJ vs TT vs 99",
            "player_cards": [("As", "Ah"), ("Ks", "Kh"), ("Qs", "Qh"), ("Js", "Jh"), ("Ts", "Th"), ("9s", "9h")],
            "board_cards": ["unknown", "unknown", "unknown", "unknown", "unknown"]
        }
    ]
    ...
    for test_case in test_cases:
        ...
        seat_count = len(test_case['player_cards'])
        context_features = {
            'seat_count': seat_count,
            'board_card_count': 0,
            'estimated_pot': 1000,
        ...
        raw_probs = predict_scenario(
            model, metadata,
            test_case['player_cards'],
            test_case['board_cards'],
            context_features
        )
        
        prob_sum = raw_probs.sum()
        print(f"   Raw probabilities: {[f'{p:.1%}' for p in raw_probs]}")
        print(f"   Sum of probabilities: {prob_sum:.1%}")
        
        if prob_sum > 1.0:
            print(f"   ✅ Correct: Sum > 100% (multiple players can have high chances)")
        elif prob_sum < 0.5:
            print(f"   ⚠️  Low sum: {prob_sum:.1%} (model might be too conservative)")
        else:
            print(f"   ℹ️  Sum: {prob_sum:.1%}")
...
def main():
    ...
    # Explain the normalization issue
    explain_normalization_issue()
    
    # Test clear winner scenarios
    results = test_clear_winner_scenarios()
    
    # Test probability sum analysis
    test_probability_sum_analysis()
    
    print(f"\n🎯 RECOMMENDATIONS:")
    print("=" * 60)
    print("1. ❌ REMOVE NORMALIZATION from the app")
    print("2. ✅ Display raw probabilities as 'Win Probability'")
    print("3. ✅ Compare probabilities directly (higher = better)")
    print("4. ✅ Add explanation that only one player wins the pot")
    print("5. ✅ Consider showing 'Equity' instead of 'Win Probability'")
