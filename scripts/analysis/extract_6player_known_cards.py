#!/usr/bin/env python3
"""
Extract 6-player hands where ALL players' cards are known at showdown.
This is the ideal dataset for training a win probability model.
"""
...
def extract_6player_known_cards(input_file: str, output_file: str, max_hands: int = 10000000):
    """Extract 6-player hands with all cards known at showdown."""
    print(f"🔍 Extracting 6-player hands with ALL cards known")
        ...
        for i, line in enumerate(infile):
            ...
            try:
                ...
                if showdown_actions:
                    ...
                    # Check if ALL 6 players have known cards
                    if len(known_cards_in_hand) >= 6:
                        hands_with_known_cards += 1
                        hands_6player_all_known += 1
                        ...
                        if len(extracted_hands) % 100 == 0:
                            print(f"  Found {len(extracted_hands)} 6-player hands with all cards known...")
                        
                        # Limit to first 10000 hands
                        if len(extracted_hands) >= 10000:
                            break
            ...
            if i % 100000 == 0:
                ...
                print(f"    6-player all known: {hands_6player_all_known}")
    ...
    print(f"  ✅ 6-player hands with all cards known: {hands_6player_all_known:,}")
    print(f"  📈 6-player showdown rate: {100*hands_with_showdown/total_hands:.3f}%")
    print(f"  📈 6-player all known rate: {100*hands_6player_all_known/total_hands:.3f}%")
    ...
    if extracted_hands:
        ...
        analyze_extracted_6player_data(extracted_hands)
        
        # Create summary
        summary = {
            "total_hands_processed": total_hands,
            "hands_with_showdown": hands_with_showdown,
            "hands_6player_all_known": hands_6player_all_known,
            "extracted_hands": len(extracted_hands),
            "processing_time_seconds": total_time,
            "sample_hands": extracted_hands[:3]
        }
        
        summary_file = Path(output_file).parent / "6player_summary.json"
...
def analyze_extracted_6player_data(hands):
    """Analyze the extracted 6-player hands data."""
    print(f"\n📈 Analyzing 6-Player Hands Data")
    ...
    print(f"  🃏 Total 6-player hands: {len(hands)}")
...
def main():
    parser = argparse.ArgumentParser(description="Extract 6-player hands with all cards known")
    parser.add_argument("--input", type=str, default="data/raw/poker_training_data.jsonl",
                       help="Input JSONL file")
    parser.add_argument("--output", type=str, default="data/raw/6player_known_cards.jsonl",
